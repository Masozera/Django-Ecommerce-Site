...
from .models import WelcomeItem, Item, OrderItem, Order, DeliveryOption, BillingAddress, Payment, Coupon, Refund
...
class RequestRefundView(View): 
    ...
    def post (self,*args, **kwargs):
        ...
        if form.is_valid():
            ...
            # edit the order ie get the order based on the reference code
            try:
                order                   = Order.objects.get(ref_code=ref_code)
                order.refund_requested  = True
                order.save()
            # Then store or own custom refund
                refund         = Refund()
                refund.order   = order
                refund.email   = email
